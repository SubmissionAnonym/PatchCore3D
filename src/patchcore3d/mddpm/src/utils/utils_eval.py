...
def _test_step(self, final_volume, data_orig, data_seg, data_mask, batch_idx, ID, label_vol) :
        ...
        if not self.cfg.resizedEvaluation: # in case of full resolution evaluation 
            final_volume = F.interpolate(final_volume, size=data_orig.shape[2:], mode="trilinear",align_corners=True).squeeze() # resize
        else: 
       ...
       # Calculate Reconstruction errors with respect to anomal/normal regions
        ...
        diff_volume = F.interpolate(diff_volume[None, ...], size=(70, 70, 45), mode="trilinear", align_corners=True)[0, 0].numpy()
        save(diff_volume, f'/workspace/data/patchcore_all/mddpm_preds/{ID[0]}.npy.gz', compression=1)        
