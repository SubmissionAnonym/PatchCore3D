...
def Train(csv, cfg, preload=False):
    subjects = []
...
def Eval(csv, cfg):
    subjects = []
    for _, sub in tqdm(csv.iterrows(), desc='prepare test set', total=len(csv)):

        isnan = False
        if type(sub.mask_path) == float:
            isnan = np.isnan(sub.mask_path)

        if sub.mask_path is not None and not isnan and tio.ScalarImage(sub.img_path, 
                                                                            reader=sitk_reader).shape != tio.ScalarImage(
        ...
        if sub.mask_path != None and not isnan:  # if we have masks
            subject_dict['mask'] = tio.LabelMap(sub.mask_path, reader=sitk_reader)
