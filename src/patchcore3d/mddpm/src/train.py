...
def train(cfg: DictConfig) -> Optional[float]: 
    ...
    base = 'logs' #cfg.callbacks.model_checkpoint.monitor # naming of logs
    if 'early_stop' in cfg.callbacks:
        base_es = 'logs' #cfg.callbacks.early_stop.monitor # early stop base metric
    ...
    for fold in [0]: #range(start_fold,end_fold): # iterate over folds 
    ...
    # Evaluate model on test set, using the best or last model from each trained fold 
        ...
        if cfg.get("test_after_training"): # and not 'simclr' in  cfg.model._target_.lower():
            ...
            preds_dict = {}
            preds_dict = {'test':{}} # a dict for each data set
            
            sets = {
                    't2':['Datamodules_train.IXI', 'Datamodules_eval.Brats21'],
                   }
            
            print(cfg.datamodule.cfg.testsets)    
