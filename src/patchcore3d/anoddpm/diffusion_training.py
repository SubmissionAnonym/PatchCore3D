...
# ROOT_DIR = "/Users/frolova/work/patchcore_all/anoddpm_results/"
# IXI_DIR = "/Users/frolova/work/patchcore_all/ixi/"
ROOT_DIR = "/workspace/data/anoddpm_results/"
IXI_DIR = "/workspace/data/ixi/"
BRATS_DIR = "/workspace/data/brats2021"
...
def train(training_dataset_loader, testing_dataset_loader, args, resume):
    ...
    iters = range(10 // args['Batch_Size']) if args["dataset"].lower() != "cifar" else range(200)
    ...
    for epoch in tqdm(tqdm_epoch):
        ...
        for i in tqdm(iters, desc=f'epoch={epoch}, loss={cur_loss}'):
            data = next(training_dataset_loader)
            images = data["images"]
            for image_ind, x in enumerate(images):
                x = x.to(device)
                ...
                if epoch % 50 == 0 and i == 0 and image_ind == 0:
...
def training_outputs(diffusion, x, est, noisy, epoch, row_size, ema, args, save_imgs=False, save_vids=False):
    ...
    if save_imgs:
        if epoch % 100 == 0:
            ...
            plt.title(f'real,sample,prediction x_0_epoch={epoch}_t={t}')
        else:
        ...
        plt.savefig(f'{ROOT_DIR}diffusion-training-images/ARGS={args["arg_num"]}/EPOCH={epoch}.png')
        plt.clf()
    if save_vids:
        fig, ax = plt.subplots()
        if epoch % 500 == 0:
...
def main():
    ...
    """
    # make directories
    for i in [f'{ROOT_DIR}model/', f"{ROOT_DIR}diffusion-videos/", f'{ROOT_DIR}diffusion-training-images/']:
        try:
            os.makedirs(i)
        except OSError:
            pass
    ...
    # load the json args
    with open(f'./test_args/{file}', 'r') as f:
        args = json.load(f)
    args['arg_num'] = file[4:-5]
    args = defaultdict_from_json(args)

    # make arg specific directories
    for i in [f'{ROOT_DIR}model/diff-params-ARGS={args["arg_num"]}',
              f'{ROOT_DIR}model/diff-params-ARGS={args["arg_num"]}/checkpoint',
              f'{ROOT_DIR}diffusion-videos/ARGS={args["arg_num"]}',
              f'{ROOT_DIR}diffusion-training-images/ARGS={args["arg_num"]}']:
        try:
            os.makedirs(i)
        except OSError:
            pass
    ...
    # else:
        # load NFBS dataset
    training_dataset, testing_dataset = dataset.init_datasets(IXI_DIR, args)
    training_dataset_loader = dataset.init_dataset_loader(training_dataset, args)
    testing_dataset_loader = dataset.init_dataset_loader(testing_dataset, args)
    ...
    if resume:
        if resume == 1:
            checkpoints = os.listdir(f'{ROOT_DIR}model/diff-params-ARGS={args["arg_num"]}/checkpoint')
            checkpoints.sort(reverse=True)
            for i in checkpoints:
                try:
                    file_dir = f"{ROOT_DIR}model/diff-params-ARGS={args['arg_num']}/checkpoint/{i}"
            ...
            file_dir = f'{ROOT_DIR}model/diff-params-ARGS={args["arg_num"]}/params-final.pt'
            loaded_model = torch.load(file_dir, map_location=device)
    ...
    for file_remove in os.listdir(f'{ROOT_DIR}model/diff-params-ARGS={args["arg_num"]}/checkpoint'):
        os.remove(os.path.join(f'{ROOT_DIR}model/diff-params-ARGS={args["arg_num"]}/checkpoint', file_remove))
    os.removedirs(f'{ROOT_DIR}model/diff-params-ARGS={args["arg_num"]}/checkpoint')
