...
from diffusion_training import ROOT_DIR, IXI_DIR, BRATS_DIR
...
def anomalous_validation_1():
    ...
    :return:
    """
    args, output = load_parameters(device, root_dir=ROOT_DIR)
...
def anomalous_metric_calculation(results_path, dataset_type):
    """
    Iterates over 4 anomalous slices for each Volume, returning diffused video for it,
    the heatmap of that & detection method (A&B) or C
    :return:
    ...
    if dataset_type == 'ixi':
        d_set = dataset.IXIDataset(
                ROOT_DIR=IXI_DIR, img_size=args['img_size'], random_slice=False, subset='test')
    else:
        d_set = dataset.BratsDataset(
                ROOT_DIR=BRATS_DIR, img_size=args['img_size'])
    ...
    final_transform = transforms.Compose(
                    ...
                    transforms.Resize(235, transforms.InterpolationMode.BILINEAR),
                    transforms.CenterCrop((240, 155)),
                    ]
                    )
    target_size_small = (70, 70, 45)

    all_times = []
    os.makedirs(os.path.join(results_path, 'preds'), exist_ok=True)
    with torch.no_grad():
        for i in tqdm(range(d_set_size)):

            new = next(loader)
            full_prediction = np.zeros((240, 240, 155))
            print(new["filenames"][0], new['slice_indices'][0][0], new['slice_indices'][0][-1])
            if os.path.exists(os.path.join(results_path, 'preds', f'{new["filenames"][0]}.npy.gz')):
                continue
            start_time = time()
            for slice_id in tqdm(new['slice_indices'][0], desc=new["filenames"][0]):
                if slice_id in new['slice_indices']:          
                    image = new["images"][i].to(device).reshape(1, 1, *args["img_size"])
                    ...
                    mse = (image - output).square().cpu().numpy()[0, 0]
                    mse = np.array(final_transform(mse))

                    full_prediction[:, slice_id, :] = mse

            all_times.append(time() - start_time)
            print('Time, min:', all_times[-1] / 60)
            small_prediction = F.interpolate(
                torch.tensor(full_prediction)[None, None, ...], size=target_size_small, mode="trilinear", align_corners=False
            )[0, 0].numpy()
            # print(full_prediction.shape, small_prediction.shape)

            save(small_prediction, os.path.join(results_path, 'preds', f'{new["filenames"][0]}.npy.gz'), compression=1)
    save_json(all_times, os.path.join(results_path, 'times.json'))
...
def roc_data():
    sys.argv[1] = "28"
    args_simplex, output_simplex = load_parameters(device, root_dir=ROOT_DIR)
    sys.argv[1] = "27"
    args_hybrid, output_hybrid = load_parameters(device, root_dir=ROOT_DIR)
    sys.argv[1] = "26"
    args_gauss, output_gauss = load_parameters(device, root_dir=ROOT_DIR)
    ...
    sys.argv[1] = "103"
    args_GAN, output_GAN = load_parameters(device, root_dir=ROOT_DIR)
...
#     plt.rcParams['font.sans-serif'] = prop.get_name()
    ...
    if str(sys.argv[1]) == "101" or str(sys.argv[1]) == "102" or str(sys.argv[1]) == "103" or str(sys.argv[1]) == \
        ...
        anomalous_metric_calculation(results_path=ROOT_DIR, dataset_type=str(sys.argv[2]))
