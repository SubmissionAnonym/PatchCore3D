...
def init_datasets(ROOT_DIR, args):
    training_dataset = IXIDataset(
            ROOT_DIR=ROOT_DIR, img_size=args['img_size'], random_slice=args['random_slice'], subset='train'
            )
    testing_dataset = IXIDataset(
            ROOT_DIR=ROOT_DIR, img_size=args['img_size'], random_slice=args['random_slice'], subset='val'
            )
...
class IXIDataset(Dataset):
    ...
    def __init__(self, ROOT_DIR, transform=True, img_size=(32, 32), random_slice=False, test_size=0.2, subset='train'):
        """
        Args:
            ROOT_DIR (string): Directory with all the images.
            transform (callable, optional): Optional transform to be applied
        ...
        self.id_dataset = Chain(
            self.id_dataset,
            Filter(lambda id: id in set(cur_ids)),
            # ScaleMRI(),
        )
    ...
    def __getitem__(self, idx):
        ...
        if self.random_slice:
            # slice_idx = randint(32, 122)
            slice_indices = np.random.choice(range(40, 100), size=10, replace=False)
        else:
            slice_indices = np.arange(image.shape[1])[image.sum(axis=(0, 2)) > 0]

        images = [image[:, slice_idx, :].astype(np.float32) for slice_idx in slice_indices]

        if self.do_transform:
            images = [self.transform(image) for image in images]
        else:
            images = [self.test_transform(image) for image in images]

        sample = {'images': images, "filenames": uid, "slice_indices": slice_indices}
        return sample
...
class BratsDataset(Dataset):
    ...
    def __init__(self, ROOT_DIR, img_size=(32, 32)):
        """
        Args:
            ROOT_DIR (string): Directory with all the images.
            transform (callable, optional): Optional transform to be applied
        ...
        self.id_dataset = BraTS2021(ROOT_DIR)
        self.all_ids = list(self.id_dataset.ids)
