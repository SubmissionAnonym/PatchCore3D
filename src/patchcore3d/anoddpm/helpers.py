...
def load_checkpoint(param, use_checkpoint, device, root_dir):
    """
    ...
    if not use_checkpoint:
        return torch.load(f'{root_dir}model/diff-params-ARGS={param}/params-final.pt', map_location=device)
    else:
        checkpoints = os.listdir(f'{root_dir}model/diff-params-ARGS={param}/checkpoint')
        checkpoints.sort(reverse=True)
        for i in checkpoints:
            try:
                file_dir = f"{root_dir}model/diff-params-ARGS={param}/checkpoint/{i}"
...
def load_parameters(device, root_dir):
    """
    ...
    if len(sys.argv[1:]) > 0:
        ...
        params = os.listdir(f"{root_dir}model")
    ...
    for param in params:
        if param.isnumeric():
            output = load_checkpoint(param, use_checkpoint, device, root_dir=root_dir)
        elif param[:4] == "args" and param[-5:] == ".json":
            output = load_checkpoint(param[4:-5], use_checkpoint, device, root_dir=root_dir)
        elif param[:4] == "args":
            output = load_checkpoint(param[4:], use_checkpoint, device, root_dir=root_dir)
        else:
