...
class RescaleSegmentor:
    def __init__(self, device, target_size=(160, 160, 103)):
        self.device = device
...
class MahalanobisDistanceScorer(object):
    def __init__(self) -> None:
        """
        Mahalanobis Distance Anomaly Scorer class.
        """
        self.feature_merger = ConcatMerger()
    ...
    def fit(self, detection_features: List[np.ndarray]) -> None:
        """Calls the fit function of the mahalanobis distance method.
        ...
        cov_mahal = np.zeros((detection_features.shape[1], detection_features.shape[1]))
        for train_sample in detection_features:
            cov_mahal += np.outer(train_sample - self.mean_mahalanobis, train_sample - self.mean_mahalanobis)

        cov_mahal /= len(detection_features)
    ...
    @staticmethod
    def _mean_values_file(folder, prepend=""):
        return os.path.join(folder, prepend + "mean_mahalanobis.npy")

    @staticmethod
    def _inv_cov_file(folder, prepend=""):
        return os.path.join(folder, prepend + "inv_cov_mahal.npy")
    ...
    def save(
        ...
        save(self.mean_mahalanobis, self._mean_values_file(save_folder, prepend))
        save(self.inv_covariance_mahalanobis, self._inv_cov_file(save_folder, prepend))
    ...
    def load(self, load_folder: str, prepend: str = "") -> None:
        self.mean_mahalanobis = load(self._mean_values_file(load_folder, prepend))
        self.inv_covariance_mahalanobis = load(self._inv_cov_file(load_folder, prepend))
