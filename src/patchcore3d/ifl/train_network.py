...
from preprocessing import img_extended
...
def main_function(experiment_directory, continue_from):
    ...
    logging.info( "Device found: " + device.type )

    L = get_spec_with_default(specs, "L", None)
    decoder = arch.Decoder(latent_size, L = L, **specs["NetworkSpecs"]).to(device)
    ...
    for epoch in tqdm(range(start_epoch, num_epochs + 1)):
        ...
        if epoch == 1:
            ...
            cur_loss = np.mean(loss_log[-len(loader):])

        for batch in tqdm(loader, desc=f'epoch={epoch}, loss={cur_loss}'):

            # Process the input data
            # TODO Move this manipulation to data loader
            xyz = torch.from_numpy(batch.xyz).to(device).permute(0,2,1).reshape(-1,3)
            pixel_gt = torch.from_numpy(batch.s).to(device).flatten()
            
            indices = torch.from_numpy(batch.k.repeat(samples_per_scene)).to(device)

            num_samples = batch.xyz.shape[0]
            ...
            # NN optimization
            pred_logp_class = decoder(input)

            batch_loss = loss_fn_class(pred_logp_class, pixel_gt.to(device).long()) / num_samples
            ...
            logging.debug("loss = {}".format(batch_loss.item()))
...
if __name__ == "__main__":
    ...
    arg_parser = argparse.ArgumentParser(description="Train a Implicit Fields autodecoder")
