...
from patchcore3d.amcons.models import Encoder, Decoder
from patchcore3d.amcons.losses import kl_loss


class AnomalyDetectorAMCons:
    def __init__(self, dir_results, device, item=['flair'], zdim=32, lr=1*1e-4, input_shape=(1, 224, 224), epochs_to_test=25,
    ...
    def train(self, train_generator, epochs, test_dataset):
        ...
        for self.i_epoch in range(self.epochs):
            ...
            for self.i_iteration, (x_n, y_n, x_a, y_a) in enumerate(self.train_generator):
                ...
                # Move tensors to gpu
                x_n = torch.tensor(x_n).to(self.device).float()
                ...
                if 'BRATS' in train_generator.dataset.dir_datasets or\
                   ...
                   'PhysioNet' in train_generator.dataset.dir_datasets:
                    ...
                    x_mask = torch.tensor(np.array(np.argwhere(x_mask > 0.5))).to(self.device).squeeze()
    ...
    def predict_score(self, x):
        ...
        z, z_mu, z_logvar, f = self.E(torch.tensor(x).to(self.device).float().unsqueeze(0))
        ...
        self.E.train()
        self.Dec.train()
        return score, anomaly_map, xhat, time_elapsed
