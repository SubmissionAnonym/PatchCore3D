...
from patchcore3d.amcons.utils import image_normalization
    ...
    def __init__(self, dir_dataset, item, partition,  input_shape=(1, 224, 224), channel_first=True, norm='max',
        ...
        if 'BRATS' in dir_dataset:
            for iModality in np.arange(0, len(self.item)):
                x = Image.open(
                    Path(self.dir_dataset) / self.item[iModality] / 'train' / 'benign' / 'IXI023-Guys-0699_77.jpg')
                x = np.asarray(x)
        ...
        for subdirs in ['benign', 'malign']:
            for images in os.listdir(Path(self.dir_dataset) / self.item[0] / self.partition / subdirs):
                if 'Thumbs.db' not in images:
                    self.images.append(Path(self.dir_dataset) / 'modality' / self.partition / subdirs / images)

        # Get number of patients (volumes), and number of slices
        if 'BRATS' in dir_dataset:
            patients = ['_'.join(str(image).split('/')[-1].split('_')[:-1]) for image in self.images]
            print('patients', patients[:4], len(patients))
        else:
            patients = [str(image).split('/')[-1][:-6] for image in self.images]
        self.unique_patients = np.unique(patients)
        slices_per_volume = len(patients) // len(self.unique_patients)

        print('slices_per_volume', slices_per_volume)
        print('unique patients', self.unique_patients[:4], len(self.unique_patients))
        ...
        for iPatient in tqdm(np.arange(0, len(self.unique_patients))):

            slices_patient = list(np.sort([iSlice for iSlice in self.images if self.unique_patients[iPatient] in str(iSlice)]))
            ...
            for iSlice in np.arange(0, slices_per_volume):
                    ...
                    for iModality in np.arange(0, len(self.item)):
                        ...
                        # Load image
                        x = Image.open(str(slices_patient[iSlice]).replace('modality', self.item[iModality]))
                        x = np.asarray(x)
                        ...
                        # Load mask
                        if 'malign' in str(slices_patient[iSlice]):
                            mask_id = str(slices_patient[iSlice]).replace('malign', 'ground_truth').replace('modality', self.item[iModality])
...
class TestDatasetForInference(object):
    ...
    def __init__(self, dir_dataset, item, partition,  input_shape=(1, 224, 224), channel_first=True, norm='max',
        ...
        for iPatient in tqdm(np.arange(0, len(self.unique_patients))):
            ...
            slices_patient = [str(iSlice).replace('modality', self.item[iModality]) for iSlice in self.images if self.unique_patients[iPatient] in str(iSlice)]

            slices_patient = sorted(slices_patient, key=lambda x: int(str(x).split('_')[-1][:-4]))
            self.X_paths.append(slices_patient)

    def get_slice(self, iPatient, iSlice):
        # Load image
        cur_path = self.X_paths[iPatient][iSlice]
        x = Image.open(cur_path)
        x = np.asarray(x)
        uid = '_'.join(cur_path.split('/')[-1].split('_')[:-1])
        ...
        # Load mask
        if 'malign' in cur_path:
            mask_id = cur_path.replace('malign', 'ground_truth').replace('modality', self.item[iModality])
            ...
            m = np.zeros_like(x, dtype=np.uint8)
            y = 0
        return x, m, y, uid
    ...
    def __init__(self, dir_datasets, modalities, input_shape=(3, 512, 512),  channel_first=True, norm='max',
        ...
        if 'BRATS' in dir_datasets:
            for iModality in np.arange(0, len(modalities)):
                x = Image.open(Path(self.dir_datasets) / modalities[iModality] / 'train' / 'benign' / 'IXI023-Guys-0699_77.jpg')
                x = np.asarray(x)
        ...
        # Get train images
        train_images = os.listdir(Path(self.dir_datasets) / modalities[0] / name_normal)
        # Remove other files
        train_images = [train_images[i] for i in range(train_images.__len__()) if train_images[i] != 'Thumbs.db']
        for iImage in train_images:
            self.train_images.append(str(Path(self.dir_datasets) / 'modality' / name_normal / iImage))

        # Get train images
        test_images = os.listdir(Path(self.dir_datasets) / modalities[0] / name_anomaly)
        # Remove other files
        test_images = [test_images[i] for i in range(test_images.__len__()) if test_images[i] != 'Thumbs.db']
        for iImage in test_images:
            self.test_images.append(str(Path(self.dir_datasets) / 'modality' / name_anomaly / iImage))
